...
//        pos--;
        ...
        do {
            try {
                chr = read();// read a byte, if none, chr == -1
            } catch (IOException e) {
                chr = -1;
            }
        } while ((chr == CR) || (chr == LF));
        ...
        pos--;  //first one which is not '\r' or '\n'

        // Reading the method name
        int maxRead = requestLine.method.length;    //当前method字符数组的长度
        int readStart = pos;
        int readCount = 0;
...
//            }
            if (pos >= count) {
                int val = read();   //pos>=count buffer内容不够用了，再读一些
                if (val == -1) {    //没了，出错
